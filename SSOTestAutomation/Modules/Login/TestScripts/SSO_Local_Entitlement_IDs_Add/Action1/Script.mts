'###############################################################################################################
'Test Script Name                          :  SSO_Local_Entitlement_IDs_Add
'Test Objective/Description         	   :  To Login and Logout into OSP Application
'Test Case ID                              :  SSO_Local_Entitlement_IDs_Add
'Test Case Name                            :  SSO_Local_Entitlement_IDs_Add
'Author                                    :  Anil Kumar V
'Designed Date                             :  7 Jan 2016
'Last Modified By                          :  
'Last Modified Date                        :  
'###############################################################################################################
...
'****************************************Setting the Project Name in the environment variable*******************************************************
                ...
                environment("StrTestCaseID")="SSO_Local_Entitlement_IDs_Add"
                strModuleName= environment("strModuleName")				

                strTestCaseName= environment("TestName")
                environment("strTestCaseName")="SSO_Local_Entitlement_IDs_Add"
                StrStartTime=now
                StrBeginTimer=Timer
...
'#################################### TEST  CASE VALIDATION STARTS HERE ##################################################################################
                ...
                DataTable.ImportSheet environment("strDataFile"), "OSP_Local_Entitlement", "Global"
...
'							Obj_Lnk_Edit_Dshbrd.Click
							...
							If ubound(SAID_val)<1 Then
								...
								If obj_Wbtbl_cnt.Exist Then
									...
									If Row_No>0 Then
										Func_CreateRow strDtlRptFileName,strTestCaseName, "Local Ent ID sucessfully added into the list " & obj_Wbtbl_cnt.Getcelldata(Row_No,1),"Pass",now	
										else
										Func_CreateRow strDtlRptFileName,strTestCaseName, "Local Ent ID is Invalid","Pass",now	
									End If
								 End If
								...
								For k = 0 To ubound(SAID_val) Step 1
									...
									If obj_Wbtbl_cnt.Exist Then
										...
										If Row_No>0 Then
											...
											Func_CreateRow strDtlRptFileName,strTestCaseName, "Local Ent ID is Invalid" &SAID_val(k),"Fail",now
										End If
									End If
								Next
							End If
